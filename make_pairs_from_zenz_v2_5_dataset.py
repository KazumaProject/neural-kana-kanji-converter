...
class MeCabAnalyzer(Analyzer):
    ...
    def __init__(self) -> None:
        try:
            ...
            raise RuntimeError(
                "fugashi not installed. Install: pip install fugashi[unidic-lite]"
            ) from e
...
# -------------------------
# Dataset iteration (zenz-v2.5-dataset)
# -------------------------
def iter_zenz_v2_5_outputs(
    streaming: bool,
    split: str,
) -> Iterable[str]:
    try:
    ...
    ds = load_dataset("Miwa-Keita/zenz-v2.5-dataset", split=split, streaming=streaming)
...
def iter_zenz_v2_5_sentences(*args: Any, **kwargs: Any) -> Iterable[str]:
    for txt in iter_zenz_v2_5_outputs(*args, **kwargs):
...
def main() -> None:
    ...
    with open(args.out, "w", encoding="utf-8") as fo:
        for sent in iter_zenz_v2_5_sentences(
